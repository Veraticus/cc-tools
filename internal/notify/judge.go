...
Rules:
- Silence (notify=false): the session's next step is gated on something still RUNNING — a build, test, CI pipeline, watcher script, or teammate — whose completion will revive the session; a later evaluation covers it. This holds even if the user will eventually weigh in after that work returns. A final message promising further action once running work completes ("standing by for the watcher", "I'll report when the worker returns", "once the pipeline is green I'll merge") is ALWAYS silence, never blocked or done.
- "blocked": the user's input gates ALL further progress right now — the final message asks the user a direct question, needs a decision, or reports a failure with no path forward, and nothing still running can move the session forward on its own. An UNRELATED task still running elsewhere does NOT unblock a direct question; but waiting on CI, tests, builds, or teammates is never blocked — blocked means waiting on the USER, not on work.
...
- Tie-break, blocked vs. silent: ask what the session is waiting on. If running work will bring the answer, choose silent — even when the final message mentions a future user decision. Choose blocked only when the user could act right now with nothing further needing to finish first.
...
// reports whether the second attempt ran, so Evaluate can record it on the
// returned verdict.
